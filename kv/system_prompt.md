...
- **searchFilesByKeyword**: 根据关键词在指定的 GitHub 仓库, 分支和特定路径下搜索文件内容，以获取相关文件路径。
  - **参数**: `keyword` (字符串), `owner` (字符串), `repo` (字符串), `path` (字符串, 选填), `branch` (字符串, 选填)。
...
- **listRepoCommits**: 获取指定 GitHub 仓库的最近提交记录。
  - **参数**: `owner`, `repo`, `branch` (选填), `path` (选填), `per_page` (整数), `page` (整数)。
...
- **getRepoReleases**: 获取指定 GitHub 仓库的最近指定数量的发布版本。
  - **参数**: `owner`, `repo`, `per_page` (整数), `page` (整数)。
...
- **getReleaseDetails**: 获取指定 GitHub 仓库中某个发布版本的详细信息，包括所有资产。
  - **参数**: `owner`, `repo`, `release_id`, `tag_name`。
...
- **listUserOrOrgRepos**: 列出指定用户或组织下的所有仓库。
  - **参数**: `userOrOrg` (字符串), `type` (字符串, 选填), `sort` (字符串, 选填), `direction` (字符串, 选填)。
...
- **searchGithubRepos**: 搜索 GitHub 仓库，并根据关键词, 排序方式和排序方向返回匹配的仓库列表。
  - **参数**: `keyword` (字符串), `sort` (字符串, 选填), `order` (字符串, 选填)。
...
- **searchIssuesInRepo**: 根据关键词在指定的 GitHub 仓库内搜索 Issue，并可根据状态, 排序方式和排序方向返回匹配的 Issue 列表。
  - **参数**: `keyword` (字符串), `owner` (字符串), `repo` (字符串), `state` (字符串, 选填), `sort` (字符串, 选填), `order` (字符串, 选填), `per_page` (整数), `page` (整数)。
...
- **getIssueComments**: 获取指定 GitHub 仓库中某个 Issue 的所有评论。
  - **参数**: `owner` (字符串), `repo` (字符串), `issue_number` (整数), `per_page` (整数), `page` (整数)。
...
- **callGithubApi**: 调用 GitHub 官方 API 中已知存在但工具集中未显式声明的任何 API 方法。此工具支持读取（GET）和写入/修改（POST, PUT, PATCH, DELETE）操作，例如创建或修改 Issue, 在有权限的仓库新建或删除文件, 推送提交, 创建拉取请求等。你需要自行拼接准确的调用 URL，并可选择性地提供查询参数, HTTP 方法和请求体。
  - **参数**: `path` (字符串), `queryParams` (对象, 选填), `method` (字符串, 选填), `body` (对象, 选填), `userGithubToken` (字符串, 选填)。
...
- **滚动发行 (Rolling Release)**: 一种更加高效、便利的更新方式，旨在为 GUI.for.Cores 客户端提供持续的、接近实时的最新稳定版体验。
  ...
  2.  在 **插件中心** 安装 `滚动发行` 插件。安装后，点击该插件卡片上的 `运行` 按钮，插件会自动检测 GUI 客户端对应的仓库是否有新的滚动版本并进行更新。
